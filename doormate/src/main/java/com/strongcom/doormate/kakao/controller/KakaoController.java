...
public class KakaoController {
    ...
    @PostMapping("/add")
    public ResponseEntity<Message> getKakaoUser(@RequestBody KakaoGetTokenDto kakaoGetTokenDto) throws Exception {
        KakaoGetUserDto kakaoUser = kakaoService.createKakaoUser(kakaoGetTokenDto.getAccessToken());
        Message message = kakaoService.joinKakaoUser(kakaoGetTokenDto.getTargetToken(),
    ...
    public ResponseEntity<Message> getUserName(@RequestHeader HttpHeaders token, @RequestBody KakaoSetUserNameDto kakaoSetUserNameDto) throws Exception {
        String authentication = token.getFirst("Authorization");
        KakaoGetUserDto kakaoUser = kakaoService.createKakaoUser(authentication);
        Message message = kakaoService.setUserName(kakaoUser.getKakaoId(), kakaoSetUserNameDto.getUserName());
